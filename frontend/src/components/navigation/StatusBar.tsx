...
export function StatusBar() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-30 border-t border-border bg-background/90 backdrop-blur-md">
      <div className="mx-auto flex h-10 items-center justify-between px-4 max-w-[1600px]">
        <div className="flex items-center gap-4 text-[10px] font-mono text-muted-foreground">
          {primaryWallet && (
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-primary" />
              <span className="tabular-nums">
                {truncateAddress(primaryWallet.address, { separator: "…" })}
              </span>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-4 text-[10px] font-mono uppercase tracking-[0.2em] text-muted-foreground">
          <a
            href="https://twitter.com/sovry_xyz"
            target="_blank"
            rel="noopener noreferrer"
            className="hover:text-foreground transition-colors"
            aria-label="Sovry on Twitter"
          >
            <Twitter className="h-3.5 w-3.5" />
          </a>
          <a
            href="https://discord.gg/RHKEjmGhNt"
            target="_blank"
            rel="noopener noreferrer"
            className="hover:text-foreground transition-colors"
            aria-label="Sovry Discord"
          >
