...
class SovryWorker {
  ...
  private async safeRun(label: string, fn: () => Promise<unknown>) {
    try {
      ...
      console.error(`ÔØî [WORKER] ${label} failed:`, error);
    }
  }

  async initializeCache() {
    console.log('ÔÜí Using in-memory cache for real-time updates');
  ...
  async updateIPPrice() {
    try {
      console.log('­ƒöä [BACKGROUND] Starting IP price update process...');
      const ipPrice = await storyscanService.getIPPriceWithFallback();
      if (ipPrice === null) {
        console.error('ÔØî [BACKGROUND] Failed to get IP price from StoryScan');
      ...
      console.log(`­ƒÆ¥ [BACKGROUND] Cached in memory: price:IP:USD = ${ipPrice}`);
      return { success: true, price: ipPrice, timestamp: new Date().toISOString(), source: 'StoryScan API' };
    } catch (error: any) {
      console.error('ÔØî [BACKGROUND] Error in IP price update:', error);
  ...
  async processPoolsRequest() {
    try {
      console.log('­ƒîÉ [USER REQUEST] Processing /api/pools request...');
      const poolsData = await getPoolsFromGoldsky();
      if (!poolsData || poolsData.length === 0) {
        console.warn('ÔÜá´©Å [USER REQUEST] No pools data from Goldsky');
      ...
      console.error('ÔØî [USER REQUEST] Error processing pools request:', error);
  ...
  async start() {
    if (this.isRunning) {
      console.log('ÔÜá´©Å Worker is already running');
      return;
    }
    console.log('­ƒÜÇ Starting Sovry Backend Worker...');
    const cacheInitialized = await this.initializeCache();
    if (!cacheInitialized) {
      console.error('ÔØî Failed to start worker - Cache initialization failed');
      return;
    }
    this.isRunning = true;
    ...
    console.log('Ô£à Sovry Backend Worker started successfully');
  }

  async stop() {
    console.log('­ƒøæ Stopping Sovry Backend Worker...');
    ...
    console.log('Ô£à Sovry Backend Worker stopped');
  }
  ...
  async forceUpdatePrice() {
    console.log('­ƒöä Manual price update triggered...');
